...
def solve_ik_xyz_down(
    ...
    # 若姿态偏差超阈值，则拒绝该步，保持上一帧关节，避免下降过程中“边转边降”。
...
def main():
    ...
    touch_down_fz_threshold = 15.0

    # 回放限速：每个 IK 点跑多个物理子步，且按 timestep sleep，防止“一下播完”。
    substeps_per_waypoint = 6
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        for i in range(1, step_num + 1):
            ...
            # 一旦 Fz 超过阈值，认为已触底，立刻中断下压轨迹。
            if wrench[5] > touch_down_fz_threshold:
                print(
                    f"[touchdown] step={i:03d}, Fz={wrench[5]:+.4f}N > {touch_down_fz_threshold:.1f}N, break descending."
                )
                break
