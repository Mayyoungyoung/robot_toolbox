...
def get_ee_contact_wrench_world(model, data, ee_body_ids, ee_pos_world):
    ...
    for i in range(data.ncon):
        ...
        # 【核心修复 2】必须使用矩阵转置 .T，将接触坐标系正确转换到世界坐标系
...
def main():
    ...
    # --- 轨迹参数调整 ---
    step_num = 900 # 【修改】更密集的插值，减小每次下压导致的单步冲击力
    distance_m = -0.45
    ...
    touch_down_fz_threshold = 4.0  # 【修改】触碰阈值，10N 左右通常是刚接触的力
    substeps_per_waypoint = 2       # 【修改】每个轨迹点跑的物理帧数
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        for i in range(1, step_num + 1):
            ...
            # 保持夹爪闭合。如果运行中夹爪还是松开了，可以尝试将 0.0 改成 -0.01 或更强烈的闭合指令
            data.ctrl[7] = 0.0 
            ...
            # 如果检测到触底，冻结机械臂当前姿态，并跳出整个下压轨迹循环
            if touchdown_detected:
                # 把真实物理位置作为新的控制目标，防止累积的位置误差继续驱使机械臂往下压
                data.ctrl[:arm_dof] = data.qpos[:arm_dof].copy()
                break 

            # 为了控制打印信息的刷屏速度，每 50 步打印一次日志
            if (i % 50 == 0) or (i == 1):
